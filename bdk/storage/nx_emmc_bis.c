...
static bool bis_file_based = false;
...
static int nx_emmc_bis_write_block(u32 sector, u32 count, void *buff, bool flush)
{
	...
	if (bis_file_based)
		res = emummc_storage_file_based_write(system_part->lba_start + sector, count, bis_cache->dma_buff) ? 0 : 1;
	else if (!emu_offset)
...
static int nx_emmc_bis_read_block_normal(u32 sector, u32 count, void *buff)
{
	...
	// If not reading from cache, do a regular read and decrypt.
	if (bis_file_based)
		res = emummc_storage_file_based_read(system_part->lba_start + sector, count, bis_cache->dma_buff) ? 0 : 1;
	else if (!emu_offset)
...
static int nx_emmc_bis_read_block_cached(u32 sector, u32 count, void *buff)
{
	...
	if (bis_file_based)
		res = emummc_storage_file_based_read(system_part->lba_start + cluster_sector, BIS_CLUSTER_SECTORS, bis_cache->dma_buff) ? 0 : 1;
	else if (!emu_offset)
...
void nx_emmc_bis_init_file_based(emmc_part_t *part, bool enable_cache, const char *base_path)
{
	...
	emummc_storage_file_base_set_partition(0); // GPP
