...
static int _dump_emummc_file_part(emmc_tool_gui_t *gui, char *sd_path, sdmmc_storage_t *storage, const emmc_part_t *part)
{
	...
	{
		s_printf(gui->txt_buf, "\n#FFDD00 Espaço livre insuficiente para emuMMC em arquivo!#\n");
	...
	while (totalSectors > 0)
	{
		...
		{
			s_printf(gui->txt_buf, "\n#FFDD00 A criação da emuMMC foi cancelada!#\n");
			lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
			manual_system_maintenance(true);
		...
		while (sdmmc_storage_read(storage, lba_curr, num, buf))
		{
			...
			{
				s_printf(gui->txt_buf, "#FFDD00 Tentando novamente...#");
				lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
				manual_system_maintenance(true);
			}
		}
...
out_failed:
	...
	if (!res)
	{
		s_printf(txt_buf, "Tempo total: %dm %ds.\nConcluído!", timer / 60, timer % 60);
		...
		s_printf(txt_buf, "Tempo total: %dm %ds.", timer / 60, timer % 60);
...
static int _dump_emummc_raw_part(emmc_tool_gui_t *gui, int active_part, int part_idx, u32 sd_part_off, emmc_part_t *part, u32 resized_count)
{
	...
	s_printf(gui->txt_buf, "#96FF00 Pasta base:#\n%s\n#96FF00 Offset da partição:# #FF8000 0x%08X#",
	...
	if (resized_count)
	{
		...
		if (!user_part)
		{
			s_printf(gui->txt_buf, "\n#FFDD00 Partição USER não encontrada!#\n");
			lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
			manual_system_maintenance(true);
	...
	if (resized_count)
	{
		...
		s_printf(gui->txt_buf, "Gravando nova GPT... ");
		lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
		manual_system_maintenance(true);
...
error:
		...
		lv_mbox_set_text(mbox, "#C7EA46 Geração de chaves BIS#");
		...
		lv_label_set_text(lb_desc, "#FFDD00 Validação das chaves BIS falhou!#\n");
...
void dump_emummc_raw(emmc_tool_gui_t *gui, int part_idx, u32 sector_start, u32 resized_count)
{
	...
	{
		s_printf(gui->txt_buf, "#FFDD00 Para formatar a partição USER,#\n#FFDD00 as chaves BIS são necessárias!#\n");
		lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
		emmc_end();
		goto out;
	}
