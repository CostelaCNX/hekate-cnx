...
static void _fm_update_status(void)
{
	...
	if (fm.has_clip)
		s_printf(clip, "%s %s", fm.clip_cut ? "Mover:" : "Copiar:", fm.clip);
	else
		strcpy(clip, "Vazio");

	if (fm.has_sel)
		s_printf(buf, "#C7EA46 Selecionado:# %s%s   #888888 |#   %s", fm.sel, fm.sel_is_dir ? "/" : "", clip);
	else
		s_printf(buf, "Nada selecionado   #888888 |#   %s", clip);
...
static lv_res_t _fm_view_save(lv_obj_t *kb)
{
	...
	_fm_view_exit_edit();
	_fm_msg(ok ? "#96FF00 Arquivo salvo.#" : "#FFDD00 Falha ao salvar!#");
...
static void _fm_view_toggle_edit(void)
{
	...
	{
		_fm_msg("#FFDD00 Arquivo grande demais para editar.#");
		return;
	}
...
static void _fm_view_text(void)
{
	if (sd_mount())
	{
		_fm_msg("#FFDD00 Falha ao iniciar o SD!#");
		return;
	}
	...
	{
		free(fp);
		_fm_msg("#FFDD00 Falha ao abrir o arquivo!#");
		return;
	}
	...
	lv_win_add_btn(win, NULL, SYMBOL_EDIT" Editar", _fm_view_edit_btn);
	fm_view_win = win;
	fm_view_kb = NULL;
	fm_view_editing = false;
...
static void _fm_refresh(void)
{
	...
	if (sd_mount())
	{
		lv_label_set_text(fm.path_lbl, "#FFDD00 Falha ao iniciar o SD!#");
		_fm_update_status();
		return;
	}
...
static lv_res_t _fm_open_action(lv_obj_t *btn)
{
	if (!fm.has_sel)
	{
		_fm_msg("#FFDD00 Nada selecionado.#");
		return LV_RES_OK;
	}
...
{
	_fm_open_keyboard(SYMBOL_DIRECTORY"  Digite o nome da nova pasta:", "", FM_KB_NEWDIR);
...
static lv_res_t _fm_rename_action(lv_obj_t *btn)
{
	...
	_fm_open_keyboard(SYMBOL_EDIT"  Digite o novo nome:", fm.sel, FM_KB_RENAME);
...
static lv_res_t _fm_delete_confirm_action(lv_obj_t *btns, const char *txt)
{
	...
	bool do_delete = !strcmp(txt, "Apagar");
	lv_obj_del(dark_bg);
	...
	if (do_delete && fm.has_sel)
	{
		if (!sd_mount())
		{
			...
			if (res != FR_OK)
				_fm_msg("#FFDD00 Falha ao apagar!#");
		}
		else
		{
			_fm_msg("#FFDD00 Falha ao iniciar o SD!#");
...
static lv_res_t _fm_delete_action(lv_obj_t *btn)
{
	...
	static const char *mbox_btn_map[] = { "\222Apagar", "\222Cancelar", "" };
	lv_obj_t *mbox = lv_mbox_create(dark_bg, NULL);
	lv_mbox_set_recolor_text(mbox, true);
	lv_obj_set_width(mbox, LV_HOR_RES / 9 * 5);
...
static void _fm_open_tools(void)
{
	lv_obj_t *win = nyx_create_standard_window(SYMBOL_LIST"  Ferramentas de Arquivo", _fm_tools_close);
	fm_tools_win = win;
	...
	if (fm.has_sel)
		s_printf(buf, "#C7EA46 Selecionado:# %s%s\n#888888 Pasta:# %s", fm.sel, fm.sel_is_dir ? "/" : "", fm.cwd);
	else
		s_printf(buf, "#FFDD00 Nenhum item selecionado.# Segure um item para escolher.\n#888888 Pasta:# %s", fm.cwd);
	lv_label_set_text(lbl, buf);
	lv_obj_align(lbl, NULL, LV_ALIGN_IN_TOP_LEFT, LV_DPI / 2, LV_DPI / 4);

	_fm_tool_btn(win, lbl, 0, SYMBOL_OK"  Selecionar",          _fm_tw_select);
	_fm_tool_btn(win, lbl, 1, SYMBOL_RIGHT"  Abrir",         _fm_tw_open);
	_fm_tool_btn(win, lbl, 2, SYMBOL_DIRECTORY"  Nova Pasta", _fm_tw_newfolder);
	_fm_tool_btn(win, lbl, 3, SYMBOL_COPY"  Copiar",          _fm_tw_copy);
	_fm_tool_btn(win, lbl, 4, SYMBOL_EDIT"  Recortar",           _fm_tw_cut);
	_fm_tool_btn(win, lbl, 5, SYMBOL_SAVE"  Colar",         _fm_tw_paste);
	_fm_tool_btn(win, lbl, 6, SYMBOL_KEYBOARD"  Renomear",    _fm_tw_rename);
	_fm_tool_btn(win, lbl, 7, SYMBOL_TRASH"  Apagar",       _fm_tw_delete);
	_fm_tool_btn(win, lbl, 8, SYMBOL_REFRESH"  Atualizar",    _fm_tw_refresh);
}
...
lv_res_t create_win_files(lv_obj_t *btn)
{
	static bool task_created = false;

	lv_obj_t *win = nyx_create_standard_window(SYMBOL_SD" Arquivos", NULL);
	lv_obj_t *parent = win;
	...
	lv_label_set_static_text(hint, "#888888 Toque numa pasta ou arquivo de texto para abrir "SYMBOL_DOT" Segure um item para as ferramentas#");
	...
	lv_label_set_text(status_lbl, "Nada selecionado");
	lv_obj_align(status_lbl, list, LV_ALIGN_OUT_BOTTOM_LEFT, 0, LV_DPI / 6);
	fm.status_lbl = status_lbl;

	// The long-press watcher only acts while a list press is pending, so a
	// single shared task is safe even after the window is closed.
	if (!task_created)
	{
