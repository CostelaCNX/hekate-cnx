...
lv_obj_t *create_window_autoboot(const char *win_title)
{
	...
	close_btn = lv_win_add_btn(win, NULL, SYMBOL_CLOSE" Fechar", _win_autoboot_close_action);
...
{
	lv_obj_t *win = create_window_autoboot(SYMBOL_GPS" Inicialização automática");
	lv_win_add_btn(win, NULL, SYMBOL_POWER" Desativar", _autoboot_disable_action);
	...
	lv_label_set_static_text(label_txt, "Configurações principais");
	...
	lv_label_set_static_text(label_txt3, "Configurações da pasta ini");
...
static lv_res_t _save_nyx_options_action(lv_obj_t *btn)
{
	...
	if (res)
		lv_mbox_set_text(mbox, "#FF8000 Configuração da interface#\n\n#96FF00 A configuração foi salva no cartão SD!#");
	else
...
{
	lv_obj_t *win = nyx_create_standard_window(SYMBOL_COPY" Tema de Cores", _action_win_nyx_colors_close);
	lv_win_add_btn(win, NULL, SYMBOL_SAVE" Salvar e recarregar", _action_win_nyx_colors_save);
	...
	lv_label_set_static_text(acc_label, "Cor de destaque:");
	...
	lv_label_set_static_text(bg_label, "Cor do tema:");
	...
	lv_label_set_static_text(label_btn, SYMBOL_REFRESH" Cinza");
	...
	lv_label_set_static_text(label_btn, SYMBOL_BRIGHTNESS" Preto");
	...
	lv_label_set_static_text(label_btn, SYMBOL_LIST" Cor personalizada");
	...
	lv_label_set_static_text(lbl_sample, "Amostra do destaque:");
	...
	lv_label_set_static_text(lbl_test,
		"Texto de exemplo para visualizar a cor de destaque "
		"em uma área com várias linhas.");
...
static lv_res_t _create_mbox_clock_edit(lv_obj_t *btn)
{
	...
	static const char *mbox_btn_map[] = { "\251", "\222Concluído", "\222Cancelar", "\251", "" };
	lv_obj_t *mbox = lv_mbox_create(dark_bg, NULL);
	lv_mbox_set_style(mbox, LV_MBOX_STYLE_BG, &mbox_style);
	lv_mbox_set_recolor_text(mbox, true);
	lv_obj_set_width(mbox, LV_HOR_RES / 9 * 6);

	lv_mbox_set_text(mbox, "Informe #C7EA46 Data# e #C7EA46 Hora# para o Nyx\n"
						   "Usado em operações de arquivo e no menu.\n"
						   "Isto não altera o relógio real do HW!");
	...
	nyx_create_onoff_button(th, h1, btn_dst, SYMBOL_BRIGHTNESS" Horário de verão automático", _action_auto_dst_toggle, true);
	if (n_cfg.timedst)
...
disabled_or_cal0_issue:;
	...
	if (!error)
	{
		if (!nx_hoag)
		{
			s_printf(txt_buf,
				"Dados salvos no SD!\n"
				"Salvo em: #C7EA46 switchroot/joycon_mac.[bin/ini]#\n\n");

			bool success = true;

			// Check if pairing info was found.
			if (joycon_found == 2)
				strcat(txt_buf, "#C7EA46 Sucesso!#\n#C7EA46 Encontrados 2 de 2 dados de pareamento Joy-Con!#\n");
			else
			{
				s_printf(txt_buf + strlen(txt_buf), "#FF8000 Falhou!#\n#FF8000 Aviso:# Encontrados #FFDD00 %d de 2# dados de pareamento!\n", joycon_found);
				success = false;
			}

			// Check if pairing was done in HOS.
			if (is_l_hos && is_r_hos)
				strcat(txt_buf, "#C7EA46 Ambos os dados são baseados em HOS!#");
			else if (!is_l_hos && is_r_hos)
			{
				strcat(txt_buf, "#FF8000 Aviso:# pareamento #FFDD00 esquerdo# não é baseado em HOS!");
				success = false;
			}
			else if (is_l_hos && !is_r_hos)
			{
				strcat(txt_buf, "#FF8000 Aviso:# pareamento #FFDD00 direito# não é baseado em HOS!");
				success = false;
			}
			else
			{
				strcat(txt_buf, "#FF8000 Aviso:# #FFDD00 nenhum# pareamento é baseado em HOS!");
				success = false;
			}

			if (!success)
				strcat(txt_buf,
					"\n\n#FFDD00 Verifique se ambos Joy-Con estão conectados,#\n"
					"#FFDD00 e se foram pareados no HOS!#");

			if (cal_error)
				s_printf(txt_buf + strlen(txt_buf), "\n\n#FF8000 Aviso: falha (%d) ao obter calibração IMU!#", cal_error);
		}
		else
		{
			s_printf(txt_buf,
				"Dados salvos no SD!\n"
				"Salvo em: #C7EA46 switchroot/switch.cal#\n\n");
			strcat(txt_buf, "#C7EA46 Sucesso!#\n#C7EA46 Dados do Lite Gamepad encontrados!#\n");
		}
	}
	else
	{
		if (!nx_hoag)
			s_printf(txt_buf, "#FFDD00 Falha ao gravar pareamento Joy-Con!#\n#FFDD00 Erro: %d#", error);
		else
			s_printf(txt_buf, "#FFDD00 Falha ao obter dados do Lite Gamepad!#\n#FFDD00 Erro: %d#", error);
	}
...
static void _check_nyx_changes()
{
	if (nyx_changes_made)
	{
		...
		lv_mbox_set_text(mbox,
			"#FF8000 Configuração da interface#\n\n"
...
lv_res_t create_win_nyx_options(lv_obj_t *parrent_btn)
{
	...
	lv_obj_t *win = nyx_create_standard_window(SYMBOL_HOME" Configurações", _action_win_nyx_options_close);
	...
	lv_label_set_static_text(label_btn, SYMBOL_COPY" Tema de Cores");
	...
	lv_label_set_static_text(label_txt2, "Personalize as cores do #C7EA46 tema# e do #C7EA46 destaque#.\n");
	...
	lv_label_set_static_text(label_txt, SYMBOL_HOME" Tela inicial");
	...
	lv_label_set_static_text(label_txt2,
		"Selecione qual tela mostrar ao abrir a interface.\n"
		"#FF8000 Todas configs:# #C7EA46 combina Mais configs nos espaços vazios de Iniciar.#\n"
		"#FF8000 Iniciar / Mais configs:# #C7EA46 usa a visualização clássica dividida.#");
	lv_obj_set_style(label_txt2, &hint_small_style);
	...
	nyx_create_onoff_button(th, sw_h2, btn2, SYMBOL_GPS" Mais entradas de inicialização", _entries_columns_action, true);
	...
	lv_label_set_recolor(label_txt2, true);
	lv_label_set_static_text(label_txt2,
		"Define 5 entradas de inicialização por linha. (Padrão: 4)\n"
		"#C7EA46 Permite mostrar até 10 entradas em Iniciar#\n"
		"#C7EA46 e Mais Configs.#\n\n\n");
	lv_obj_set_style(label_txt2, &hint_small_style);
	...
	lv_label_set_static_text(label_btn3, SYMBOL_DOWNLOAD" Salvar BT Joy-Con");
	...
	lv_label_set_recolor(label_txt2, true);
	lv_label_set_static_text(label_txt2,
		"Permite salvar os endereços MAC do Switch e Joy-Con\n"
		"e as LTKs associadas. Para #C7EA46 Android# e #C7EA46 Linux#.");
	lv_obj_set_style(label_txt2, &hint_small_style);
	...
	lv_label_set_static_text(label_txt, SYMBOL_MODULES_ALT" Verificação");
	lv_obj_set_style(label_txt, th->label.prim);
	lv_obj_align(label_txt, line_sep, LV_ALIGN_OUT_BOTTOM_LEFT, LV_DPI / 4, LV_DPI / 4);
	...
	label_txt2 = lv_label_create(sw_h3, NULL);
	lv_label_set_static_text(label_txt2, "Define o tipo de verificação em backup e restauração.\n"
		"Pode ser cancelado sem perder o backup/restauração.\n");
	lv_obj_set_style(label_txt2, &hint_small_style);
	lv_obj_align(label_txt2, label_txt, LV_ALIGN_OUT_BOTTOM_LEFT, 0, LV_DPI / 4);
	...
	lv_label_set_static_text(label_btn5, SYMBOL_CLOCK" Relógio (offset)");
	...
	lv_label_set_recolor(label_txt2, true);
	lv_label_set_static_text(label_txt2,
		"Altera o offset do relógio manualmente.\n"
		"#C7EA46 A data e hora informadas serão convertidas para offset#\n"
		"#C7EA46 automaticamente. Também será usado pelo FatFS.#");
	lv_obj_set_style(label_txt2, &hint_small_style);
...
void create_tab_options(lv_theme_t *th, lv_obj_t *parent)
{
	...
	lv_btn_set_fit(btn, true, true);
	lv_btn_set_toggle(btn, true);
	lv_label_set_static_text(label_btn, SYMBOL_GPS" Inicialização auto #00FFC9   LIG #");
	...
	lv_obj_t *label_txt2 = lv_label_create(sw_h2, NULL);
	lv_label_set_static_text(label_txt2, "Escolha qual entrada de inicialização ou payload iniciar automaticamente\nao injetar.");
	lv_obj_set_style(label_txt2, &hint_small_style);
	...
	lv_obj_t *label_txt = lv_label_create(l_cont, NULL);
	lv_label_set_static_text(label_txt, SYMBOL_CLOCK" Atraso da inicialização");
	lv_obj_set_style(label_txt, th->label.prim);
	lv_obj_align(label_txt, line_sep, LV_ALIGN_OUT_BOTTOM_LEFT, LV_DPI / 4, LV_DPI / 4);
	...
	lv_label_set_recolor(label_txt2, true);
	lv_label_set_static_text(label_txt2,
		"Define por quanto tempo mostrar a logo de boot com autoboot ativo.\n"
		"#C7EA46 Pressione# #FF8000 VOL-# #C7EA46 nesse período para abrir o menu do hekate.#\n");
	lv_obj_set_style(label_txt2, &hint_small_style);
	lv_obj_align(label_txt2, label_txt, LV_ALIGN_OUT_BOTTOM_LEFT, 0, LV_DPI / 4);
	...
	lv_label_set_recolor(label_txt2, true);
	lv_label_set_static_text(label_txt2,
		"Verifica os fuses e aplica o patch automaticamente\n"
		"em firmware superior. Agora é configuração global e\n"
		"vem em auto por padrão. (LIG: Auto)\n\n\n");
	lv_obj_set_style(label_txt2, &hint_small_style);
	lv_obj_align(label_txt2, btn2, LV_ALIGN_OUT_BOTTOM_LEFT, LV_DPI / 4, LV_DPI / 12);
	...
	lv_obj_t *btn3 = lv_btn_create(sw_h3, NULL);
	nyx_create_onoff_button(th, sw_h3, btn3, SYMBOL_POWER" Desligar HOS auto", auto_hos_poweroff_toggle, true);
	lv_obj_align(btn3, label_sep, LV_ALIGN_OUT_BOTTOM_LEFT, 0, 0);

	label_txt2 = lv_label_create(sw_h3, NULL);
	lv_label_set_static_text(label_txt2,
		"Quando Desligar é usado no HOS, o console acorda após\n"
		"15s. Ative para desligar automaticamente na próxima\ninjeção de payload.");
	lv_obj_set_style(label_txt2, &hint_small_style);
	...
	label_txt = lv_label_create(sw_h3, NULL);
	lv_label_set_static_text(label_txt, SYMBOL_BRIGHTNESS" Brilho");
	lv_obj_set_style(label_txt, th->label.prim);
	lv_obj_align(label_txt, line_sep, LV_ALIGN_OUT_BOTTOM_LEFT, LV_DPI / 4, LV_DPI / 4);
	...
	label_txt2 = lv_label_create(sw_h3, NULL);
	lv_label_set_static_text(label_txt2, "Define o brilho da tela.\n\n");
	lv_obj_set_style(label_txt2, &hint_small_style);
	lv_obj_align(label_txt2, label_txt, LV_ALIGN_OUT_BOTTOM_LEFT, 0, LV_DPI / 4);
	...
	nyx_create_onoff_button(th, sw_h3, btn4, SYMBOL_REFRESH" Atualizar reboot para payload", _update_r2p_action, true);
	lv_obj_align(btn4, line_sep, LV_ALIGN_OUT_BOTTOM_LEFT, 0, LV_DPI / 10);

	label_txt2 = lv_label_create(sw_h3, NULL);
	lv_label_set_recolor(label_txt2, true);
	lv_label_set_static_text(label_txt2,
		"Se #FF8000 FSS0# for usado na entrada de inicialização selecionada, o binário\n"
		"reboot para payload será verificado e atualizado para hekate.\n\n\n\n");
	lv_obj_set_style(label_txt2, &hint_small_style);
