...
static u32 emummc_file_resized_cnt;
...
static void _create_window_emummc()
{
	...
	lv_obj_t *win;
	if (!mbr_ctx.part_idx)
		win = nyx_create_standard_window(SYMBOL_DRIVE"  Criar emuMMC em arquivo", _action_emummc_window_close);
	else
		win = nyx_create_standard_window(SYMBOL_DRIVE"  Criar emuMMC em partição", _action_emummc_window_close);
	...
	if (!mbr_ctx.part_idx)
	{
		dump_emummc_file(&emmc_tool_gui_ctxt, emummc_file_resized_cnt);
		emummc_file_resized_cnt = 0;
	}
	else
...
static void _create_mbox_emummc_raw()
{
	...
	{
		s_printf(txt_buf,
			"#C7EA46 Partição(ões) compatível(is) encontrada(s)!#\n"
			"#FF8000 Escolha uma partição para continuar:#\n\n");
	}
	else
		s_printf(txt_buf, "#FFDD00 Nenhuma partição aplicável encontrada!#\n\n");

	s_printf(txt_buf + strlen(txt_buf),
		"Tabela de partições:\n"
		"#C0C0C0 Part 0: Tipo: %02x, Início: %08x, Tamanho: %08x#\n"
		"#%s Part 1: Tipo: %02x, Início: %08x, Tamanho: %08x#\n"
		"#%s Part 2: Tipo: %02x, Início: %08x, Tamanho: %08x#\n"
		"#%s Part 3: Tipo: %02x, Início: %08x, Tamanho: %08x#",
	...
	if (mbr_ctx.resized_cnt[0] || mbr_ctx.resized_cnt[1] || mbr_ctx.resized_cnt[2])
		strcat(txt_buf, "\n\n#FFDD00 Nota:# Entradas amarelas tem a partição USER reduzida.");

	if (!mbr_ctx.available)
		strcat(txt_buf, "\n#FF8000 Deseja particionar o cartão SD?#\n"
						  "#FF8000 (Você escolherá como prosseguir)#");
...
static lv_res_t _create_emummc_file_based_action(lv_obj_t *btns, const char *txt)
{
	...
	{
		emummc_file_resized_cnt = (slider_ctx.emu_size != slider_ctx.full_size)
			? slider_ctx.emu_size << 11  // MB → sectors
			: 0;
...
static void _create_mbox_emummc_file_based()
{
	...
	static const char *mbox_btns[]    = { "\222Continuar", "\222Cancelar", "" };
	static const char *mbox_btns_ok[] = { "\251", "\222OK", "\251", "" };

	lv_mbox_set_text(mbox, "#C7EA46 Selecionar tamanho do emuMMC!#\n\n"
	                       "#00DDFF Status:# Verificando espaço disponível...");
	lv_obj_align(mbox, NULL, LV_ALIGN_CENTER, 0, 0);
	manual_system_maintenance(true);

	emmc_initialize(false);
	u32 value_full = emmc_storage.sec_cnt >> 11; // sectors → GiB (x2)
	emmc_end();
	...
	// Espaço livre em MB, menos 1 GiB de buffer de segurança
	u32 available = (sd_fs.free_clst * sd_fs.csize) >> 11; // sectors → GiB-ish (MB)
	...
	if (!available)
	{
		s_printf(txt_buf, "#C7EA46 Selecionar tamanho do emuMMC!#\n\n"
		                  "#FFDD00 Atenção:# Espaço insuficiente no SD.");
		lv_mbox_set_text(mbox, txt_buf);
	}
	else if (value_full > available)
	{
		s_printf(txt_buf, "#C7EA46 Selecionar tamanho do emuMMC!#\n\n"
		                  "#FFDD00 Atenção:# Espaço insuficiente para emuMMC completo.\n"
		                  "A partição USER será redimensionada e formatada.");
		lv_mbox_set_text(mbox, txt_buf);
	}
	else
	{
		lv_mbox_set_text(mbox, "#C7EA46 Selecionar tamanho do emuMMC!#\n\n"
		                       "#FFDD00 Nota:# Qualquer valor diferente de \"COMPLETO\" formatará a USER.");
	}
	free(txt_buf);
	...
	if (max_slide >= EMU_FILE_SLIDE_MIN)
	{
		...
		// Estilos do slider usando as cores do tema
		static lv_style_t bar_bg, bar_ind, bar_btn;
		lv_style_copy(&bar_bg,  lv_theme_get_current()->bar.bg);
		lv_style_copy(&bar_ind, lv_theme_get_current()->bar.indic);
		lv_style_copy(&bar_btn, lv_theme_get_current()->slider.knob);
...
static lv_res_t _create_mbox_emummc_create(lv_obj_t *btn)
{
	...
	static const char * mbox_btn_map[] = { "\222Arquivo SD", "\222Partição SD", "\222Cancelar", "" };
	lv_obj_t * mbox = lv_mbox_create(dark_bg, NULL);
	lv_mbox_set_recolor_text(mbox, true);
	lv_obj_set_width(mbox, LV_HOR_RES / 9 * 6);

	lv_mbox_set_text(mbox,
		"Bem-vindo a ferramenta de criação de #C7EA46 emuMMC#!\n\n"
		"Escolha o tipo de emuMMC que deseja criar.\n"
		"#FF8000 Arquivo SD# salva arquivos na partição FAT.\n"
		"#FF8000 Partição SD# salva uma imagem raw em uma partição disponível.");
...
static void _create_emummc_migrated_mbox()
{
	...
	lv_mbox_set_text(mbox,
		"#FF8000 Configuração da emuMMC#\n\n"
		"#96FF00 A configuração da emuMMC#\n#96FF00 foi salva no cartão SD!#");
...
static lv_res_t _create_emummc_migrate_action(lv_obj_t * btns, const char * txt)
{
	...
	if (backup)
	{
		if (!emummc_backup)
			s_printf(txt_buf,
				"#C7EA46 Backup de eMMC compatível encontrado!#\n\n"
				"#FF8000 Deseja migrá-lo?#\n");
		else
			s_printf(txt_buf,
				"#C7EA46 Backup de emuMMC compatível encontrado!#\n\n"
				"#FF8000 Deseja migrá-lo?#\n");
	...
	{
		s_printf(txt_buf,
			"#C7EA46 emuMMC em partição SD encontrada!#\n\n"
			"#FF8000 Deseja reparar a configuração e o tipo da partição?#\n");
		lv_mbox_add_btns(mbox, mbox_btn_map, _create_emummc_mig0_action);
	}
	else
	{
		s_printf(txt_buf, "Nenhuma emuMMC encontrada!\n");
...
static lv_res_t _create_mbox_emummc_migrate(lv_obj_t *btn)
{
	...
	static char *mbox_btn_map[] = { "\262Backup", "\262Corrigir RAW", "\222Cancelar", "" };
	lv_obj_t * mbox = lv_mbox_create(dark_bg, NULL);
	lv_mbox_set_recolor_text(mbox, true);
	lv_obj_set_width(mbox, LV_HOR_RES / 9 * 6);

	lv_mbox_set_text(mbox,
		"Bem-vindo a ferramenta de migração #C7EA46 emuMMC#!\n\n"
		"Escolha o tipo de migração que deseja fazer.\n"
		"O que não for encontrado tera o botão desativado.");
...
{
	lv_obj_t *win = nyx_create_standard_window(SYMBOL_SETTINGS"  Alterar emuMMC", _action_win_change_emummc_close);
	lv_win_add_btn(win, NULL, SYMBOL_POWER"  Desativar", _save_disable_emummc_cfg_action);
...
out0:;
	...
	lv_label_set_static_text(label_txt, "Partições RAW no SD");
	...
	for (u32 raw_btn_idx = 0; raw_btn_idx < 3; raw_btn_idx++)
	{
		...
		s_printf(txt_buf, "Setor inicial: 0x%08X\nPasta: %s", emummc_img->part_sector[raw_btn_idx], &emummc_img->part_path[raw_btn_idx * 128]);
	...
	lv_label_set_static_text(label_txt3, "Arquivos no SD");
...
{
	lv_obj_t *win = nyx_create_standard_window(SYMBOL_EDIT"  Gerenciar emuMMC", NULL);
	...
	lv_obj_t *label_txt = lv_label_create(h1, NULL);
	lv_label_set_static_text(label_txt, "Info e seleção de emuMMC");
	lv_obj_set_style(label_txt, lv_theme_get_current()->label.prim);
	...
	lv_label_set_static_text(label_btn, emu_info.enabled ? "#96FF00 "SYMBOL_OK"  Ativada!#" : "#FF8000 "SYMBOL_CLOSE"  Desativada!#");
	...
	{
		if (emu_info.sector)
			s_printf(txt_buf, "#00DDFF Tipo:# Partição RAW no SD\n#00DDFF Setor:# 0x%08X\n#00DDFF Pasta Nintendo:# %s\n",
				emu_info.sector, emu_info.nintendo_path ? emu_info.nintendo_path : "");
		else
			s_printf(txt_buf, "#00DDFF Tipo:# Arquivo no SD\n#00DDFF Pasta base:# %s\n#00DDFF Pasta Nintendo:# %s\n",
	...
	{
		lv_label_set_static_text(label_txt2, "A emuMMC está desativada e a eMMC será usada no boot.\n\n\n");
	}
	...
	lv_label_set_static_text(label_btn, SYMBOL_SETTINGS"  Alterar emuMMC");
	...
	lv_label_set_static_text(label_txt2,
		"Escolha entre imagens criadas na pasta emuMMC\n"
		"ou em partições do cartão SD. E possível ter ate 3\n"
		"emuMMC por partição e várias em arquivos.");
	...
	lv_obj_t *label_txt3 = lv_label_create(h2, NULL);
	lv_label_set_static_text(label_txt3, "Ferramentas emuMMC");
	lv_obj_set_style(label_txt3, lv_theme_get_current()->label.prim);
	...
	lv_label_set_static_text(label_btn, SYMBOL_DRIVE"  Criar emuMMC");
	...
	lv_label_set_static_text(label_txt4,
		"Permite criar uma nova emuMMC em #C7EA46 arquivo no SD# ou\n"
		"#C7EA46 partição RAW no SD#. Pode usar eMMC ou backup.\n");
	...
	lv_label_set_static_text(label_btn, SYMBOL_SHUFFLE"  Migrar emuMMC");
	...
	lv_label_set_recolor(label_txt4, true);
	lv_label_set_static_text(label_txt4,
		"Migra um backup para #C7EA46 emuMMC em arquivo no SD# ou repara\n"
		"uma #C7EA46 emuMMC em partição RAW no SD# existente.\n");
	lv_obj_set_style(label_txt4, &hint_small_style);
