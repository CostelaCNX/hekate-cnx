...
static int _emmc_sd_copy_verify(emmc_tool_gui_t *gui, sdmmc_storage_t *storage, u32 lba_curr, const char *outFilename, const emmc_part_t *part)
{
	...
	if (f_open(&fp, outFilename, FA_READ) == FR_OK)
	{
		if (n_cfg.verification == 3)
		{
			...
			if (res)
			{
				...
				s_printf(gui->txt_buf,
						"\n#FF0000 Não foi possível gravar o arquivo de hash (erro %d)!#\n"
						"#FF0000 Abortando..#\n", res);
		...
		while (totalSectorsVer > 0)
		{
			...
			if ((n_cfg.verification >= 2) || !(sparseShouldVerify % 4))
			{
				...
				{
					s_printf(gui->txt_buf,
						"\n#FF0000 Falha ao ler %d blocos (@LBA %08X),#\n"
						"#FF0000 da eMMC! Verificação falhou..#\n",
						num, lba_curr);
					lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
					manual_system_maintenance(true);
				...
				if (f_read_fast(&fp, bufSd, num << 9))
				{
					s_printf(gui->txt_buf,
						"\n#FF0000 Falha ao ler %d blocos (@LBA %08X),#\n"
						"#FF0000 do cartão SD! Verificação falhou..#\n",
						num, lba_curr);
					lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
					manual_system_maintenance(true);
				...
				{
					s_printf(gui->txt_buf,
						"\n#FF0000 Dados do SD e da eMMC (@LBA %08X) não conferem!#\n"
						"\n#FF0000 Verificação falhou..#\n",
						lba_curr);
					lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
					manual_system_maintenance(true);
			...
			{
				strcpy(gui->txt_buf, "#FFDD00 A verificação foi cancelada!#\n");
				lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
				manual_system_maintenance(true);
	...
	{
		strcpy(gui->txt_buf, "\n#FFDD00 Arquivo não encontrado ou não pôde ser carregado!#\n#FFDD00 Verificação falhou..#\n");
		lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
		manual_system_maintenance(true);
...
static int _dump_emmc_part(emmc_tool_gui_t *gui, char *sd_path, int active_part, sdmmc_storage_t *storage, emmc_part_t *part)
{
	...
	if (gui->raw_emummc)
	{
		...
		{
			strcpy(gui->txt_buf, "\n#FFDD00 Falha ao encontrar uma partição...#\n");
			lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
			manual_system_maintenance(true);
	...
	s_printf(gui->txt_buf, "#96FF00 Espaço livre no cartão SD:# %d MiB\n#96FF00 Tamanho total do backup:# %d MiB\n\n",
	...
	if (totalSectors > (sd_fs.free_clst * sd_fs.csize))
	{
		...
		strcpy(gui->txt_buf, "\n#FFBA00 O espaço livre é menor que o tamanho do backup.#\n");
		lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
		manual_system_maintenance(true);

		if (!maxSplitParts)
		{
			strcpy(gui->txt_buf, "#FFDD00 Espaço livre insuficiente para backup parcial!#\n");
			lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
			manual_system_maintenance(true);
	...
	{
		strcpy(gui->txt_buf, "\n#AEFD14 Backup parcial em andamento. Continuando...#\n");
		lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
		manual_system_maintenance(true);
		...
		if (!maxSplitParts)
		{
			strcpy(gui->txt_buf, "\n#FFDD00 Espaço livre insuficiente para backup parcial!#\n");
			lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
			manual_system_maintenance(true);
	...
	{
		s_printf(gui->txt_buf, "\n#FFBA00 Backup parcial ativado (partes de %d MiB)...#\n", multipartSplitSize >> 20);
		lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
		manual_system_maintenance(true);
	}
	...
	if (!f_open(&fp, outFilename, FA_READ))
	{
		...
		lv_obj_t *warn_mbox_bg = create_mbox_text(
			"#FFDD00 Um backup existente foi detectado!#\n\n"
			"Pressione #FF8000 POWER# para continuar.\nPressione #FF8000 VOL# para abortar.", false);
		manual_system_maintenance(true);
	...
	s_printf(gui->txt_buf, "#96FF00 Caminho:#\n%s\n#96FF00 Arquivo:# #FF8000 %s#",
	...
	while (totalSectors > 0)
	{
		if (numSplitParts != 0 && bytesWritten >= multipartSplitSize)
		{
			...
			if (isSmallSdCard)
			{
				...
				{
					strcpy(gui->txt_buf, "\n#FF0000 Erro ao criar o arquivo partial.idx!#\n");
					lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
					manual_system_maintenance(true);
				...
				{
					create_mbox_text(
						"#96FF00 Backup parcial em andamento!#\n\n"
						"#96FF00 1.# Pressione OK para desmontar o cartão SD.\n"
						"#96FF00 2.# Remova o cartão SD e mova arquivos para liberar espaço.\n"
						"#FFDD00 Não mova o arquivo partial.idx!#\n"
						"#96FF00 3.# Insira o cartão SD novamente.\n"
						"#96FF00 4.# Selecione a MESMA opção para continuar.", true);
		...
		while (res_read)
		{
			...
			{
				s_printf(gui->txt_buf,
					"\n#FFDD00 Erro ao ler %d blocos @ LBA %08X,#\n"
					"#FFDD00 da eMMC (tentativa %d). #",
			...
			else
			{
				s_printf(gui->txt_buf,
					"\n#FFDD00 Erro ao ler %d blocos @ LBA %08X,#\n"
					"#FFDD00 da emuMMC @ %08X (tentativa %d). #",
			...
			if (retryCount >= 3)
			{
				strcpy(gui->txt_buf, "#FF0000 Abortando...#\nTente novamente...\n");
				lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
				manual_system_maintenance(true);
			...
			{
				strcpy(gui->txt_buf, "#FFDD00 Tentando novamente...#\n");
				lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
				manual_system_maintenance(true);
			}
		...
		if (res)
		{
			s_printf(gui->txt_buf, "\n#FF0000 Erro fatal (%d) ao gravar no cartão SD#\nTente novamente...\n", res);
			lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
			manual_system_maintenance(true);
	...
	if (isSmallSdCard)
	{
		...
		create_mbox_text(
			"#96FF00 Backup parcial concluído!#\n\n"
			"Agora você pode juntar os arquivos se precisar\ne obter o backup RAW GPP completo da eMMC.", true);
...
void dump_emmc_selected(emmcPartType_t dumpType, emmc_tool_gui_t *gui)
{
	...
	lv_label_set_text(gui->label_info, "Verificando espaço livre disponível...");
	manual_system_maintenance(true);

	if (sd_mount())
	{
		lv_label_set_text(gui->label_info, "#FFDD00 Falha ao inicializar SD!#");
		goto out;
	}
	...
	{
		lv_label_set_text(gui->label_info, "#FFDD00 Falha ao inicializar eMMC!#");
		goto out;
	}
	...
	if (dumpType & PART_BOOT)
	{
		...
		for (i = 0; i < 2; i++)
		{
			...
			s_printf(txt_buf, "#00DDFF %02d: %s#\n#00DDFF Intervalo: 0x%08X - 0x%08X#\n\n",
			...
			if (res)
				strcpy(txt_buf, "#FFDD00 Falhou!#\n");
			else
				strcpy(txt_buf, "Concluído!\n");
	...
	if (!res && n_cfg.verification && !gui->raw_emummc)
		s_printf(txt_buf, "Tempo decorrido: %dm %ds.\n#96FF00 Concluído e verificado!#", timer / 60, timer % 60);
	else if (!res)
		s_printf(txt_buf, "Tempo decorrido: %dm %ds.\nConcluído!", timer / 60, timer % 60);
	else
		s_printf(txt_buf, "Tempo decorrido: %dm %ds.", timer / 60, timer % 60);
...
static int _restore_emmc_part(emmc_tool_gui_t *gui, char *sd_path, int active_part, sdmmc_storage_t *storage, emmc_part_t *part, bool allow_multi_part)
{
	...
	if (f_stat(outFilename, &fno))
	{
		...
		strcpy(gui->txt_buf, "\nNenhum arquivo único, procurando arquivos em partes...\n");
		lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
		manual_system_maintenance(true);
		...
		while ((u32)((u64)totalCheckFileSize >> (u64)9) != totalSectors)
		{
			...
			{
				strcpy(gui->txt_buf, "\n#FF8000 O tamanho do backup dividido no SD excede#\n#FF8000 o tamanho da partição selecionada da eMMC!#\n#FFDD00 Abortando...#");
				lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
				manual_system_maintenance(true);
			...
			else if (f_stat(outFilename, &fno))
			{
				if (!gui->raw_emummc)
				{
					s_printf(gui->txt_buf, "\n#FFDD00 Erro: arquivo não encontrado#\n#FFDD00 %s.#\n\n", outFilename);
					lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
					manual_system_maintenance(true);
				...
				if (!numSplitParts)
				{
					strcpy(gui->txt_buf, "\n#FFDD00 A pasta de restauração está vazia.#\n\n");
					lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
					manual_system_maintenance(true);
			...
			else
			{
				...
				{
					strcpy(gui->txt_buf, "\n#FFDD00 O arquivo dividido precisa ser#\n#FFDD00 múltiplo de 4 MiB.#\n#FFDD00 Abortando...#");
					lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
					manual_system_maintenance(true);
		...
		s_printf(gui->txt_buf, "%X setores no total.\n", (u32)((u64)totalCheckFileSize >> (u64)9));
		lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
		manual_system_maintenance(true);
		...
		{
			lv_obj_t *warn_mbox_bg = create_mbox_text(
				"#FF8000 O tamanho do backup dividido no SD não corresponde#\n#FF8000 ao tamanho da partição selecionada da eMMC!#\n\n"
				"#FFDD00 O backup pode estar corrompido#\n#FFDD00 ou com arquivos faltando!#\n#FFDD00 É recomendado abortar!#\n\n"
				"Pressione #FF8000 POWER# para continuar.\nPressione #FF8000 VOL# para abortar.", false);
			manual_system_maintenance(true);
			...
			{
				lv_obj_del(warn_mbox_bg);
				strcpy(gui->txt_buf, "\n#FF0000 O tamanho do backup dividido no SD não corresponde#\n#FF0000 ao tamanho da partição selecionada da eMMC!#\n");
				lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
				manual_system_maintenance(true);
...
multipart_not_allowed:
	...
	if (res)
	{
		if (res != FR_NO_FILE)
		{
			s_printf(gui->txt_buf, "\n#FF0000 Erro (%d) ao abrir arquivo. Continuando...#\n", res);
			lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
			manual_system_maintenance(true);
		}
		else
		{
			s_printf(gui->txt_buf, "\n#FFDD00 Erro (%d): arquivo não encontrado. Continuando...#\n", res);
			lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
			manual_system_maintenance(true);
		}
	...
	else if (!use_multipart && (((u32)((u64)f_size(&fp) >> (u64)9)) != totalSectors)) // Check total restore size vs emmc size.
	{
		...
		{
			strcpy(gui->txt_buf, "\n#FF8000 O tamanho do backup no SD excede#\n#FF8000 o tamanho da partição selecionada da eMMC!#\n#FFDD00 Abortando...#");
			lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
			manual_system_maintenance(true);
		...
		{
			lv_obj_t *warn_mbox_bg = create_mbox_text(
				"#FF8000 O tamanho do backup no SD não corresponde#\n#FF8000 ao tamanho da partição selecionada da eMMC!#\n\n"
				"#FFDD00 O backup pode estar corrompido!#\n#FFDD00 É recomendado abortar!#\n\n"
				"Pressione #FF8000 POWER# para continuar.\nPressione #FF8000 VOL# para abortar.", false);
			manual_system_maintenance(true);
			...
			{
				lv_obj_del(warn_mbox_bg);
				strcpy(gui->txt_buf, "\n#FF0000 O tamanho do backup no SD não corresponde#\n#FF0000 ao tamanho da partição selecionada da eMMC.#\n");
				lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
				manual_system_maintenance(true);
	...
	else
	{
		...
		s_printf(gui->txt_buf, "\nTamanho total da restauração: %d MiB.\n",
	...
	while (totalSectors > 0)
	{
		...
		if (numSplitParts != 0 && bytesWritten >= fileSize)
		{
			...
			if (res)
			{
				s_printf(gui->txt_buf, "\n#FF0000 Erro (%d) ao abrir arquivo#\n#FFDD00 %s!#\n", res, outFilename);
				lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
				manual_system_maintenance(true);
		...
		{
			s_printf(gui->txt_buf,
				"\n#FF0000 Erro fatal (%d) ao ler do SD!#\n"
				"#FF0000 Este dispositivo pode ficar inoperante!#\n"
				"#FFDD00 Tente novamente agora!#\n", res);
			lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
			manual_system_maintenance(true);
		...
		{
			s_printf(gui->txt_buf,
				"\n#FFDD00 Erro ao gravar %d blocos @ LBA %08X,#\n"
				"#FFDD00 na eMMC (tentativa %d). #",
				num, lba_curr, ++retryCount);
			lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
			manual_system_maintenance(true);
...
void restore_emmc_selected(emmcPartType_t restoreType, emmc_tool_gui_t *gui)
{
	...
	if (!gui->raw_emummc)
		strcpy(txt_buf, "#FFDD00 Isto pode deixar o dispositivo inoperante!#");
	else
		strcpy(txt_buf, "#FFDD00 Isto pode deixar a emuMMC inoperante!#");
	strcat(txt_buf, "\n\n#FFDD00 Tem certeza mesmo?#");

	if (gui->raw_emummc)
		strcat(txt_buf, "\n\nSomente a 1ª emuMMC encontrada pode ser restaurada!");

	if ((restoreType & PART_BOOT) || (restoreType & PART_GP_ALL))
	{
		strcat(txt_buf,
			"\n\nO modo selecionado só restaurará\nas partições que conseguir encontrar.\n"
			"Se uma partição não for encontrada,\nela será ignorada e a próxima continuará.");
	}
	...
	{
		s_printf(txt_buf + orig_msg_len, "\n\n#888888 Aguarde... (%ds)#", failsafe_wait);
	...
	if (!(btn_wait() & BTN_POWER))
	{
		lv_label_set_text(gui->label_info, "#FFDD00 A operação de restauração foi abortada!#");
		lv_obj_del(warn_mbox_bg);
		goto out;
	}
	lv_obj_del(warn_mbox_bg);
